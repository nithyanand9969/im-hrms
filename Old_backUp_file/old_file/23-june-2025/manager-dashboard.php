...
if ($stmt->execute()) {
    ...
    // Don't redirect - let the page reload to show modal
} else {
    $_SESSION['toast'] = ['type' => 'error', 'message' => 'Error submitting leave request: ' . $stmt->error];
}
    $stmt->close();
}
...
<div class="bg-white rounded-lg mb-10 shadow-md overflow-hidden mt-6">
...
?>
    <tr class="bg-white">
        ...
        <td class="px-6 py-4">Team <?= $user['id'] ?></td>
    ...
    <?php while ($leave = $leaves->fetch_assoc()): 
        ...
        $period = new DatePeriod($from, DateInterval::createFromDateString('1 day'), $to->modify('+1 day'));
        ...
        foreach ($period as $dt) {
            ...
            if (
                in_array(date('w', strtotime($prev)), [0, 6]) || in_array($prev, $holidays) ||
                in_array(date('w', strtotime($next)), [0, 6]) || in_array($next, $holidays)
            ) {
                $is_sandwich = true;
                break;
            }
            ...
            <div class="border p-4 rounded-md shadow-sm <?= $is_sandwich ? 'bg-red-50' : '' ?>">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <p><strong>Duration:</strong> <?= $leave['leave_duration'] ?></p>
                        ...
                        <p class="mt-2 text-sm font-semibold text-red-600">
                            тЪая╕П Sandwich Leave Detected (includes weekend or holiday)
                        </p>
                        <?php endif; ?>
                    </div>
                    <form action="process_approval.php" method="post" class="flex space-x-2">
                        <input type="hidden" name="leave_id" value="<?= $leave['id'] ?>">
                        <button type="submit" name="action" value="approve" class="bg-green-500 hover:bg-green-600 text-white px-4 py-1 rounded">Approve</button>
                        <button type="submit" name="action" value="reject" class="bg-red-500 hover:bg-red-600 text-white px-4 py-1 rounded">Reject</button>
                    </form>
                </div>
            </div>
        </td>
  ...
  <div class="bg-white rounded-lg shadow p-4 flex items-center space-x-4">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div class="bg-white p-6 shadow rounded-lg">
            ...
            <?php
            $stmt = $conn->prepare("SELECT leave_type, leave_duration, from_date, to_date, status FROM leave_requests WHERE user_id = ? ORDER BY from_date DESC");
            $stmt->bind_param("i", $user_id);
            $stmt->execute();
            $result = $stmt->get_result();
              ...
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3">
                  <span class="px-3 py-1 rounded-full text-xs font-medium <?= $statusClass ?>">
                    <i class="<?= $statusIcon ?> mr-1"></i> <?= ucfirst($leave['status']) ?>
                  </span>
                </td>
              </tr>
            <?php endforeach; ?>
