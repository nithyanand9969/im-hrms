...
<div class="bg-white p-6 shadow rounded-lg">
    ...
    <div class="overflow-x-auto">
        <table class="w-full table-auto text-sm text-gray-700">
            ...
            <?php
// Fetch leave history before rendering HTML
$leave_history = [];
...
$stmt->bind_param("i", $user_id); // make sure $user_id is from $_SESSION
$stmt->execute();
$result = $stmt->get_result();
...
?>
            ...
            <tbody class="divide-y divide-gray-200">
                <?php if (!empty($leave_history)): ?>
                    <?php foreach ($leave_history as $leave): ?>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2"><?= htmlspecialchars(date('d-m-Y', strtotime($leave['from_date']))) ?></td>
                            <td class="px-4 py-2"><?= htmlspecialchars(date('d-m-Y', strtotime($leave['to_date']))) ?></td>
                            <td class="px-4 py-2">
                ...
                <?php else: ?>
                    <tr>
                        <td colspan="5" class="text-center text-gray-400 py-4">No leave history found.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
function toggleLeaveType(isHalfDay) {
    ...
    if (isHalfDay) {
        ...
        document.getElementById('to_date').value = document.getElementById('from_date').value; // mirror date
    } else {
        toDateContainer.style.display = 'block';
        halfDayType.classList.add('hidden');
        document.getElementById('to_date').value = ''; // clear value
    }
}

// When From Date changes, also update To Date for Half Day
document.getElementById('from_date').addEventListener('change', () => {
    const halfDayRadio = document.querySelector('input[name="leave_duration"]:checked');
    if (halfDayRadio && halfDayRadio.value.toLowerCase() === 'half day') {
                ...
                data.users.forEach(user => {
                    ...
                    row.innerHTML = `
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            <button class="toggle-status-btn text-green-600 hover:text-green-800" 
