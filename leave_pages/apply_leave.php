...
$balance_updated = (int)$leave['balance_updated'] ?? 0;
...
if ($role === 'admin' && in_array($email, ['yatish@integritymatters.in', 'bhumija@integritymatters.in'])) {
    // Admin approving (could be final if applicant is admin)
    if ($leave['role'] === 'admin') {
        $is_final_approval = true;
    }
} elseif ($email === 'rachna@integritymatters.in') {
    // Rachna's approval is always final for User and Manager
    $is_final_approval = true;
}

if ($is_final_approval) {
    $stmt = $conn->prepare("UPDATE leave_requests SET status = 'approved', approval_level = ?, balance_updated = 1 WHERE id = ?");
    $stmt->bind_param("ii", $current_level, $leave_id);
    $stmt->execute();
    $stmt->close();

    applySandwichLeavePolicy($conn, $from_date, $to_date, $applicant_id, $leave_duration, $half_day_type);
} else {
...
function applySandwichLeavePolicy($conn, $from_date, $to_date, $user_id, $leave_duration, $half_day_type = null) {
    $deduct_days = 0;
    ...
    $current_balance = (float)($res['leave_balance'] ?? 0);
